...
async function runIt() {
  ...
  randomInt(1298, 2419);
  await page.type("#data-print-book-title", "HY I'M TITLE", {
    delay: 120,
  });
  ...
  randomInt(298, 419);
  await page.type("#data-print-book-subtitle", "HY I'M A SUBTITLE", {
    delay: 120,
  });
  ...
  randomInt(192, 619);
  await page.type("#data-print-book-description", "HY I'M A DESCRIPTION", {
    delay: 120,
  });
  ...
  //     [
  //       ...document.querySelectorAll("#category-chooser-popover .a-link-normal"),
  //     ].map((elem) => elem.click())
  //   );
  //   await page.evaluate(() => {
  //     document
  //       .querySelectorAll("#category-chooser-popover .a-link-normal")[1]
  //       .click();
  //   });

  await page.waitFor(randomInt(5000, 5222));
  const clickThemAll = await page.$$eval(
    "#category-chooser-popover .a-link-normal",
    (links) => links.forEach((link) => link.click())
  );
  ...
  const clickSeleted = await page.$$eval(".a-label", (links) =>
    links.forEach((el) => {
  ...
  // ! another solution (i didnt use)
  // page.waitForSelector("#free-print-isbn-btn-announce", { visible: true });
  ...
  // ! await for selector : NOT WORKING

  // ! await for XPath :
  // ! IMPORTANT data[print_book][publisher_interior][status]
  await page
    .waitForXPath(
